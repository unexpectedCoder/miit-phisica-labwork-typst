...
// Уберите текст ```...``` при финальной подготовке отчёта
```
...
На все приложения к отчёты должны быть ссылки в тексте.
В приложении~@appendix-code приведён исходный код программы для обработки измерений.
В приложении~@appendix-additional-materials содержатся дополнительные графики...
На рисунке~@appendix-fig-Schrodinger показан портрет...
...
Выводы по работе в соответствии с целью и задачами; приводятся конкретные результаты работы; делаются выводы о достижении цели и решении поставленных задач.

+ Сделано то-то.
+ Получено то-то.
+ Решена такая-то задача.
+ И так далее.

#show: appendixes.with()

// > Каждое приложение — это заголовок 1-го уровня.
//   Статус приложения указывается в скобках: (обязательное) или (справочное).
//   Используйте функцию appendix-heading(status)[Название] или просто = Название.

= Программа обработки данных <appendix-code>

В приложения выносится объёмная или второстепенная информация, приведение которой в основном тексте может усложнить целостное восприятие отчёта.
...
= Дополнительные материалы <appendix-additional-materials>

Графики, схемы, блох-схемы и прочее, что не вместилось в основной текст.

#figure(
  image("pics/Schrodinger.png", width: 50%),
  caption: [Картинка в приложении]
) <appendix-fig-Schrodinger>

Ссылки на рисунки и таблицы в приложениях работают как обычно.
